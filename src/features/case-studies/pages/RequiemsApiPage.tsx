...
export default function RequiemsApiPage() {
	const t = useTranslations("RequiemsApiPage");

	const architectureItems = t.raw("architecture.items") as Array<{
		name: string;
		role: string;
		detail: string;
	}>;
	const performanceItems = t.raw("performance.items") as Array<{
		heading: string;
		body: string;
	}>;
	const apiCategoryItems = t.raw("apiCategories.items") as Array<{
		name: string;
		detail: string;
	}>;
	...
	return (
		<div className="pt-32 pb-24">
			<div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
				...
				<motion.section {...fadeUp} className="mb-16">
					<h2 className="font-heading text-2xl font-bold text-brand-primary mb-6">
						{t("apiCategories.heading")}
					</h2>
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						{apiCategoryItems.map((cat) => (
							<div
								key={cat.name}
